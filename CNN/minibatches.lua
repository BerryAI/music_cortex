...
function minibatch(model, criterion, train_x,
                   train_y, classes, config)
   local parameters, gradParameters = model:getParameters()
   local confusion = optim.ConfusionMatrix(classes)
   config = config or {} -- pending
   local batch_size = config.xBatchSize or 12 -- pending not sure 'or' is correct
