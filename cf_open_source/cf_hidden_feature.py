...
def stochastic_GD_with_ini(rating_matrix, user_weight, lean_rate,
    ...
    for h in range (0, max_iter):
        ...
        if res_norm < 0.01:
            ...
            break
        res_norm_old = res_norm
    return user_weight, hidden_feature, full_success
...
def stochastic_GD_r(rating_matrix, lean_rate, lambda_rate, k, max_iter):
    ...
    for i in range(0,10):
        ...
        user_weight, hidden_feature, full_success = stochastic_GD_with_ini(
...
def get_hidden_feature_matrix_GD(user_rate_dict, k, lean_rate,
    ...
    user_index, song_index, rating_matrix = full_rating_matrix_with_index(
                                            user_rate_dict)
    if GD_method == 1:
        user_weight, hidden_feature = stochastic_GD(rating_matrix,
                                        lean_rate, lambda_rate, k, max_iter)
    if GD_method == 2:
        user_weight, hidden_feature = stochastic_GD_r(rating_matrix,
                                        lean_rate, lambda_rate, k, max_iter)
    if GD_method == 3:
        user_weight, hidden_feature = batch_GD(rating_matrix,
                                        lean_rate, lambda_rate, k, max_iter)
